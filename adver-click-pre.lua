...
-- urldecode end
r_url = unescape(r_url);

local url_has_gif = (string.match(r_url,"files") and string.match(r_url,"web.gif"));
...
local event1 = '{"data":[{"dt":"evt","pr":{"$ct":'..osnow..',"$tz":'..time_zone..',"$sid":'..sid..',"$url":"'..url..'","$eid":"广告点击","_项目id":"'..projectid..'","_媒体id":"'..mediaid..'","_广告位id":"'..placementid..'","_创意id":"'..creativeid..'","_redirecturl":"'..r_url..'","$utm_source":"'..utm_source..'","$utm_medium":"'..utm_medium..'","$utm_campaign":"'..utm_campaign..'","$utm_content":"'..utm_content..'","$utm_term":"'..utm_term..'"}}],"sln":"itn","pl":"js","sdk":"zg_adver","sdkv":"2.0","owner":"zg","ut":"'..ut..'","tz":'..time_zone..',"debug":'..debug..',"ak":"'..appkey..'","usr":{"did":"'..did..'"}}';
local event2 = '{"data":[{"dt":"evt","pr":{"$ct":'..osnow..',"$tz":'..time_zone..',"$sid":'..sid..',"$url":"'..url..'","$eid":"广告点击-PRE","_项目id":"'..projectid..'","_媒体id":"'..mediaid..'","_广告位id":"'..placementid..'","_创意id":"'..creativeid..'","_redirecturl":"'..r_url..'","$utm_source":"'..utm_source..'","$utm_medium":"'..utm_medium..'","$utm_campaign":"'..utm_campaign..'","$utm_content":"'..utm_content..'","$utm_term":"'..utm_term..'"}}],"sln":"itn","pl":"js","sdk":"zg_adver","sdkv":"2.0","owner":"zg","ut":"'..ut..'","tz":'..time_zone..',"debug":'..debug..',"ak":"'..appkey..'","usr":{"did":"'..did..'"}}';
local c_event = '{"event":'..cjson.encode(event2)..'}';
if url_has_gif then
    c_event = '{"event":'..cjson.encode(event1)..'}';
end
...
-- set did cookie
if has_did == nil then
    ck.set("zg_adver_did",did,"/",host_name,false,true,3600*24*365);
end

-- set session id cookie
if has_sid == nil then 
    ck.set("zg_adver_sid",sid,"/",host_name,false,true,3600*24*365);
end
-- set ak
    ck.set("zg_adver_ak",appkey,"/",host_name,false,true,3600*24*365);
-- set projectid
    ck.set("zg_adver_pjd",projectid,"/",host_name,false,true,3600*24*365);
-- set mediaid
    ck.set("zg_adver_md",mediaid,"/",host_name,false,true,3600*24*365);
-- set placementid
    ck.set("zg_adver_pld",placementid,"/",host_name,false,true,3600*24*365);
-- set creativeid
    ck.set("zg_adver_ctd",creativeid,"/",host_name,false,true,3600*24*365);
...
--重定向不传参
local zgad_url = ngx.var.scheme.."://"..ngx.var.host.."/files/zgad.html?1=1";
if appkey then
    zgad_url = zgad_url.."&appkey="..appkey;
end
if projectid then
    zgad_url = zgad_url.."&projectid="..projectid;
end
if mediaid then
    zgad_url = zgad_url.."&mediaid="..mediaid;
end
if placementid then
    zgad_url = zgad_url.."&placementid="..placementid;
end
if creativeid then
    zgad_url = zgad_url.."&creativeid="..creativeid;
end

if utm_source ~= nil and utm_source ~= "" then 
    zgad_url = zgad_url.."&utm_source="..utm_source;
end
if utm_medium ~= nil and utm_medium ~= "" then 
    zgad_url = zgad_url.."&utm_medium="..utm_medium;
end
if utm_campaign ~= nil and utm_campaign ~= "" then 
    zgad_url = zgad_url.."&utm_campaign="..utm_campaign;
end
if utm_content ~= nil and utm_content ~= "" then 
    zgad_url = zgad_url.."&utm_content="..utm_content;
end
if utm_term ~= nil and utm_term ~= "" then 
    zgad_url = zgad_url.."&utm_term="..utm_term;
end    
if url_has_gif then
...
if r_url ~= nil and r_url ~= "" then
  r_url = r_url:gsub("\n", "\r\n")
  r_url = r_url:gsub("([^%w ])", char_to_hex)
  r_url = r_url:gsub(" ", "+")
end
-- urlencode end
zgad_url = zgad_url.."&url="..r_url;
return ngx.redirect(zgad_url);
